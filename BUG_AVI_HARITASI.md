...
| 07-25 | **DB katmanı — 2. geçiş (pool + retry, analizci kategorileri)** | durum-dizili sahte-PG (commit-sonra-kopar) | **53. bug — PG kör retry sessiz ÇİFT-YAZMA.** `query()`/`execute()` her runtime_error'da bağlantıyı yenileyip sorguyu koşulsuz yeniden çalıştırıyordu. Gönderim-SONRASI kopmada (sunucu INSERT'i işledi, yanıt gelmeden koptu) retry statement'ı 2. kez çalıştırıyor → çift ödeme/sipariş, üstelik app sessiz "done" görüyor. 3 ölçüm: exception=runtime_error retry'a düşüyor ✓, in_tx kontrolü yok (autocommit+tx ikisi de bozuk) ✓, sahte-sunucu sayaç=2 ✓. Fix: retry YALNIZ `PgSendError`'da (gönderim başarısız=güvenli); yanıt-okuma kopması fail-loud propagate. MySQL zaten güvenliydi (post-send retry yok). Pool yaşam döngüsü: sağlık kontrolü yok ama lazy-reconnect ile mitige. Guard: db_retry_test.sh + fake_pg_server commit_kopar |
| 07-25 | **DB param binding (2. geçiş, kategori 4)** | float hassasiyet sondası | **Bug DEĞİL — aday çürütüldü.** Canlı yol `bind_params` (db::query/exec) float'ı `setprecision(17)` locale-bağımsız yazıyor (zaten "default 6 hane bozardı" diye düzeltilmiş); `?`-literal/yorum ayrımı ve param-sayısı uyuşmazlığında fail-loud da var. **Latent landmine:** `extended_query` DbParam yolu (postgres_client.cpp:970) `std::to_string(double)` ile 6-hane kaybediyor (`1e-10→0`) AMA LOOK'tan erişilemez (SMTP `from_text`; `from_float` çağıran yok). db::exec ileride DbParam-float yoluna bağlanırsa düzeltilmeli (precision-17'ye çevir) |
| 07-25 | **DB havuz zehri + send simetrisi (2. geçiş, kat. 1+2)** | RST-sonrası pool reuse + kaynak denetimi | **Bug DEĞİL (kanıtlanamadı) — sağlamlaştırma yapıldı.** Analizci kaynak denetiminde `send_bytes`'ın kopmada `sock_`'u INVALID YAPMADIĞINI buldu (read yolları yapıyor). Gözlemlenebilir bug ÜRETİLEMEDİ (ikimiz de): `PgSendError` retry'i `do_connect()` ile kurtarılabilir vakaları kurtarıyor; RST testi query 1'i yarışıyor. Yine de simetri hardening: `send_bytes` artık read yollarıyla tutarlı (close+INVALID) → "kapalı ama geçerli görünen fd" havuza dönmez. Kategori-1 yan sonuç: mid-response RST'de istemci **fail-loud** (çökme/hang/sessiz-bozulma yok) → sağlam. `ConnPool::release` health-check yok ama lazy-reconnect + bu simetri ile mitige. Regresyon-yok (differential+db_retry). NOT: analizcinin "d0e6191/54. bug fix" premisi yanlış — öyle bir commit/iddia yoktu |
| 07-25 | **HTTP/router — 2. geçiş (analizci: smuggling önceliği)** | raw HTTP harness (http_smuggle_test) | **54. bug — request smuggling zemini (RFC 7230 çerçeve).** Ölçüldü: (a) `Transfer-Encoding` `find("chunked")` SUBSTRING eşliyordu → `xchunked`/`chunkedx`/`chunked, gzip` chunked sanılıp gövde çözülüyordu (200/blen=5); (b) field-name'den önce boşluk (`Content-Length : 5`) sessizce kabul (RFC yasak). Katı front-end bunları farklı çerçevelerse desync. Fix: TE tam-eşleşme (`te != "chunked"` → 400); kolon-öncesi boşluk → parse ret (bağlantı kapat, keep-alive smuggle kalmaz). Guard: http_smuggle_test.sh + .py + echo_app. Static serving path-traversal: **yüzey YOK** (route/template; file:: zaten weakly_canonical+FILE_ROOT). Chunked size parse katı (hex-dışı/taşma ret). CL+TE birlikte zaten ret |
| 07-25 | **HTTP body — keep-alive/pipelining (2. geçiş)** | raw HTTP split-send | **55. bug — Content-Length üst sınır olarak zorlanmıyordu.** Body ayrı TCP segmentinde gelince body-okuma döngüsü (http_server.cpp) son `recv`'in TÜM baytlarını ekliyordu; pipeline'daki sonraki isteğin başlangıcı body'ye karışıyordu (ölçüldü: CL=10, body=24). App bozuk gövde görür + smuggling-bitişik. İlk-buffer yolu zaten `min()` ile kırpıyordu; döngü kırpmıyordu. Fix: `append(tmp, min(r, content_len - body.size()))`. Guard: http_smuggle_test body-overshoot. Keep-alive genel: reddedilen istek → `::close(fd)` (smuggled bayt kalmaz); kabul edilen artık-bayt sonraki iterasyonda `buf` sıfırlandığı için düşer (pipelining yok ama smuggling-güvenli) |
| 07-25 | **HTTP Slowloris — yavaş-okuma (2. geçiş, analizci önceliği)** | raw HTTP sürekli-damla | **56. bug — header'lar için toplam süre sınırı yoktu (Slowloris DoS).** idle timeout (fiber 15s / pool 30s SO_RCVTIMEO) her baytta SIFIRLANIYOR → istemci idle'in altında bayt/6s damlatırsa bağlantı SÜRESIZ tutuluyor; pool modunda worker thread'i tüketir (bounded pool → DoS). Ölçüldü: ~30s'lik damla 200 döndü. Fix: istek BAŞLADIKTAN sonra header'lar `LOOK_HEADER_TIMEOUT` (varsayılan 15s) içinde tamamlanmalı, aşımda 408+kapat. Keep-alive'da istekLER-ARASI bekleme dahil değil (timer ilk baytta başlar). nginx arkasında mitige, doğrudan `--mode http` açıkta savunma. Guard: http_slowloris_test (sürekli-damla). NOT: partial-sonra-sessiz varyantı zaten idle timeout ile kapanıyordu (bounded) |
| 07-25 | **HTTP sınır/ayrıştırma turu (2. geçiş, kat. devam)** | raw HTTP boundary probe | **Çoğu TEMİZ + 1 hardening (bug DEĞİL).** Ölçüldü: uzun URI (500KB)→404 bounded; **>2MB header bloğu→ConnectionReset (2MB cap ÇALIŞIYOR)**; 20000 header→404 hızlı (O(n²) yok, hang yok); bare-LF satır sonu→reddedilir (katı CRLF); bare-LF smuggle→gizli istek header bloğunda düşer, işlenmez. Yani sınır BURADA zorlanıyor (55/56'nın aksine). **Hardening:** obs-fold (SP/HTAB ile başlayan devam satırı, RFC 7230 §3.2.4 istekte yasak) sessizce düşürülüyordu → katlanmış framing header'ı desync bırakabilirdi; artık parse_request reddediyor. Kanıtlı exploit yok (front-end geçersiz katlanmış CL/TE'yi zaten reddeder) → hardening, numaralandırılmadı. Guard: http_smuggle_test obs-fold vakası |
| 07-25 | **Kategori-1 DB durum-dizili (2. geçiş — ertelenmiş kapatıldı)** | fake_pg_server durum-dizili modlar | **TEMİZ — bug yok.** Analizcinin 3 senaryosu + bellek seli ölçüldü: sirasiz (DataRow← RowDescription'dan önce)→columns boş, default kullanılır (satır 832 `i<columns.size()` bounds-check, NULL-deref YOK); erken_c (CommandComplete, hiç DataRow yok)→SURVIVED; hs_error (handshake'te ErrorResponse)→temiz exception ("test reddi"); param_seli (200000 ParameterStatus)→**6.4MB bounded, OOM yok**, temiz exception. Wire-parser sırasız/beklenmedik pakete karşı dayanıklı — önceki DataRow bounds-check + mesaj-tavanı işinin meyvesi. Guard: db_stateful_test.sh (çökme=exit≥128 tespiti). 53 (commit_kopar) bu damardan çıkmıştı; kalan senaryolar temiz → damar KAPANDI |
| 07-25 | **Motor/parite — 2. geçiş (analizci: düşmanca differential)** | tree-walk↔VM düşmanca girdi + derin özyineleme | **57. bug — VM özyineleme SINIRSIZDI (OOM DoS + parite ayrışması).** `MAX_CALL_DEPTH` kontrolü yalnız `call_closure`'daydı (host→VM: callback/route); bytecode **CALL opcode'u (vm.cpp:601,699) frame'i limitsiz push ediyordu**. Ölçüldü: `r(10000000)` VM'de döndü (tree-walk 256'da hata verir). Doğrudan LOOK özyinelemesi bellek tükenene dek gidiyordu (graceful "Stack overflow" yerine OOM). Fix: CALL yolu da `MAX_CALL_DEPTH`(500) zorlar. Guard: vm_recursion_test.sh (VM bounded + iki motor graceful). Düşmanca parite probe'u ayrıca: sayısal sınır (int64 taşma, Inf, -0, coerce) tree-walk==VM birebir; **minör kalan ayrışma:** hata mesajı formatı (tree-walk `Column` verir, VM vermez) + recursion limiti (tree-walk 256 vs VM 500) — ikisi de bounded, yürütme-modeli farkı |
| 07-25 | **Motor/parite — edge-semantik + büyük-yapı (ASan)** | düşmanca differential + ASan+UBSan | **TEMİZ — bug yok.** Edge-semantik parite birebir (tree-walk==VM): closure yakalama (use→değer "012", implicit→referans "333"), short-circuit yan etki (g çağrılmıyor), try/catch-return, array kopya (referans semantik). Büyük-yapı **ASan+UBSan** altında temiz: 100k dizi+kopya, 50k map, 100k string concat, 1000-derin nested — heap-overflow/UAF/UBSan YOK. Not: tree-walk derin özyineleme ASan-Debug'da DEADLYSIGNAL verir (büyük frame'ler 256 guard'dan önce C++ stack taşırır) ama **Release'de graceful** (deep(255)→255 exit 0 doğrulandı) → ASan-Debug artefaktı, üretim riski değil. ASan reçetesi: `dnf install gcc-toolset-12-libasan-devel libubsan-devel` + `cmake -DLOOK_SANITIZE=address,undefined -DOPENSSL_CRYPTO_LIBRARY=/usr/lib64/libcrypto.so ...` |
| 07-25 | **58. bug — implicit closure yakalama parite ayrışması (analizci buldu)** | döngü-içi yerel + implicit capture | **KANITLANDI, fix bekliyor (tasarım kararı + önemsiz-olmayan).** `while(){ $v=$i; push($fns, fn()=>$v) }` → tree-walk `0-1-2`, VM `2-2-2`. YAZILIMCI EKSİK ÖLÇTÜ: yalnız `use($i)` (012 parite) ve `fn()=>$i` döngü-değişkeni (333 parite) test etmiştim; **döngü-içi yerel değişkenin implicit yakalanmasını atladım** → "edge-semantik temiz" eksikti. Analizci çift-doğrulamada yakaladı. Kök neden: tree-walk döngü-gövdesi yereline iterasyon-başına TAZE binding (blok-scope, JS `let`); VM tek slot yeniden kullanıyor. Mutasyon testi ikisi de by-ref (10). **Hızlı fix YOK:** snapshot yaparsam döngü düzelir ama mutasyon 5'e ayrışır → closure/scoping modelini hizalamak gerek (Lua-tarzı per-iterasyon upvalue). Tasarım kararı: hangi semantik doğru (tree-walk canonical → 012). differential_body `use` formunu test ediyor (satır 90), implicit formu YOK → guard eklenmeli |
| 07-25 | **58 TAM MATRİS (analizci çift-doğrulama + closure-yazma)** | 7 vakalı closure semantik matrisi | **İKİ bağımsız ayrışma, ZIT yönde + yazma PARİTE.** A global-mut 10/10 ✓; **B fonksiyon-mut 10/5 ✗** (VM by-value snapshot, tree-walk by-ref); **C global-döngü-yerel 012/222 ✗** (VM global slot paylaşır); D fonksiyon-döngü-yerel 012/012 ✓ (VM snapshot per-iter tesadüfen tutuyor); E global-döngü-değişken 333/333 ✓; **F/G closure-YAZMA 5/5 ✓** (ikisi de yeni-local). Kök neden: VM iki farklı capture davranışı — fonksiyon-local=snapshot, global=paylaşılan-slot; tree-walk tek model=by-ref-per-iter-cell. Fix kapsamı DARALDI: yazma zaten parite → yalnız read-capture'ı by-ref-cell + per-iter binding'e hizala (yazma yeni-local kalır). Analizcinin "yazma bozulur" endişesi ölçümle elendi |
