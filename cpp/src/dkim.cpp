...
bool dkim_verify(const std::string& raw_message) {
    ...
    // RFC 6376 §2.7 / RFC 7489: d= must be the same as or a parent of the From: domain.
    // Without this check a valid sig from attacker.com could pass on a From: victim.com message.
    {
        size_t from_pos = raw_message.find("\r\nFrom:");
        if (from_pos == std::string::npos) from_pos = raw_message.find("From:");
        if (from_pos != std::string::npos) {
            size_t at = raw_message.find('@', from_pos);
