...
```bash
# 1. ZIP indir
wget -L -O look-lang.zip "https://github.com/codlook/look/releases/download/v1.0.1/look-lang-plesk-1.1.1.zip"

# 2. Aç ve kur (tek komut)
unzip -o look-lang.zip -d look-lang && sed -i "s/\r//" look-lang/setup.sh && bash look-lang/setup.sh
```

Kurulum tamamlandığında panel adresi terminale yazdırılır:
```
[LOOK] Panel: https://<plesk-ip>/modules/look-lang/
```
...
├── setup.sh                        # Kurulum scripti (ZIP açıldıktan sonra çalıştırılır)
├── scripts/
...
> **Not:** `setup.sh` Plesk kaydını, dosya kopyalamayı ve binary kurulumunu otomatik yapar.  
> `sed -i "s/\r//"` Windows'tan gelen CRLF satır sonlarını temizler.
